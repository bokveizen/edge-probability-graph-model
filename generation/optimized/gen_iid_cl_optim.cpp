...
int main(int argc, char const *argv[]) {
    ...
    vector<float50> degrees;
    vector<unsigned int> n_nodes_deg;
    vector<float50> alphas_deg;

    float50 degree;
    unsigned int n_nodes;
    float50 alpha;
    float50 total_degree = 0.0;
    ...
    vector<float50> degrees_vec;
    vector<unsigned int> degree_idx_vec;
    vector<float50> alphas;
    vector<vector<unsigned int>> degree_nodes(degrees.size());

    unsigned int node_idx = 0;
    for (unsigned int i = 0; i < degrees.size(); ++i) {
        for (unsigned int j = 0; j < n_nodes_deg[i]; ++j) {
            degrees_vec.push_back(degrees[i]);
            degree_idx_vec.push_back(i);
            alphas.push_back(alphas_deg[i]);
            degree_nodes[i].push_back(node_idx++);
        }
    }

    auto n_degrees = degrees.size();
    vector<vector<float50>> edge_probs_round(n_degrees, vector<float50>(n_degrees, 0.0));
    vector<vector<float50>> edge_probs_remain(n_degrees, vector<float50>(n_degrees, 0.0));
    for (unsigned int i = 0; i < n_degrees; ++i) {
        for (unsigned int j = i; j < n_degrees; ++j) {
            float50 edge_prob = degrees[i] * degrees[j] / total_degree;            
            if (edge_prob > 1.0) {
                edge_prob = 1.0;
            }
            // Compute the probability for each round
            float50 r_ij =
                (1 - pow(1 - edge_prob, 1.0 / n_rounds)) / (alphas_deg[i] * alphas_deg[j]);
            if (r_ij > 1.0) {
                float50 p_max = 1.0 - pow(1.0 - alphas_deg[i] * alphas_deg[j], n_rounds);
    ...
    pairs_sorted.reserve(n_degrees * (n_degrees + 1) / 2);
    probs_sorted.reserve(n_degrees * (n_degrees + 1) / 2);
    vector<unsigned long long> pairs_remain;
    pairs_remain.reserve(n_degrees * (n_degrees + 1) / 2);
    for (unsigned int i = 0; i < n_degrees; ++i) {
        for (unsigned int j = i; j < n_degrees; ++j) {
            pairs_sorted.push_back(encode_pair(i, j, n_degrees));
            probs_sorted.push_back(edge_probs_round[i][j]);
            if (edge_probs_remain[i][j] > 0.0) {
                pairs_remain.push_back(encode_pair(i, j, n_degrees));
            }
        }
    }    
    ...
    // For generation graphs, we need the following information:
    // edge_probs[i][j] = the final edge probability between a node in group i and a node in group j
    // n_nodes_deg[i] = the number of nodes in group i
    ...
    for (auto i_graph = 0; i_graph < n_graphs; i_graph++) {
        ...
        generate_edges(n_rounds, total_nodes, degree_nodes, alphas_deg, pairs_sorted, probs_sorted,
            ...
            for (const auto& clique : cliques) {
                ...
                for (unsigned int i = 0; i < size_clique; ++i) {
                    ...
                    for (unsigned int j = i + 1; j < size_clique; ++j) {
                        auto node2 = clique[j];
                        if (node1 > node2) swap(node1, node2); // Ensure node1 < node2
                        generated_edges.insert(encode_pair(node1, node2, total_nodes));
                    }
                }
            }
            // Expand bi-cliques
